...
async function getUserUsageData(userContext, orgId, period = 'current') {
  try {
    // Get subscriptions for this specific user in the organization
    let subscriptions = [];
    const userId = userContext?.userId || userContext?.sub;
    
    logger.info({ orgId, userId, userContext }, '[getUserUsageData] Starting to fetch usage data');
    
    if (!userId) {
      logger.warn({ orgId }, "[getUserUsageData] No userId provided for usage data");
      return {
        totalRequests: 0,
    ...
    try {
      subscriptions = await adminDao.listSubscriptionsByUser(orgId, userId);
      logger.info({ orgId, userId, subscriptionsCount: subscriptions?.length }, '[getUserUsageData] Subscriptions fetched');
    } catch (err) {
      logger.warn({ err, orgId, userId }, "[getUserUsageData] No subscriptions found for user");
    ...
    // Include ACTIVE, CANCELED_SCHEDULED (still active until period end), and free subscriptions (no PAYMENT_STATUS)
    const activeSubs = (subscriptions || []).filter(sub => 
      sub.PAYMENT_STATUS === 'ACTIVE' || 
      sub.PAYMENT_STATUS === 'CANCELED_SCHEDULED' || 
      !sub.PAYMENT_STATUS || 
      sub.PAYMENT_STATUS === null
    );
    
    logger.info({ orgId, userId, activeSubsCount: activeSubs.length }, '[getUserUsageData] Active subscriptions filtered');
